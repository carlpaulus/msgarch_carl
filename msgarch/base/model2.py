...
class MSGarch(LikelihoodModel):
    ...
    def log_likelihood(self, params):
        ...
        gamma = params[3 * self.k_regimes:3 * self.k_regimes + (self.k_regimes + self.k_regimes)]
        # gamma = gamma.reshape(self.k_regimes, self.k_regimes-1)
        ...
        P = np.ones((self.k_regimes, self.k_regimes))
        # for i in range(0, self.k_regimes):
        #     for j in range(0, self.k_regimes):

        # sans ambiguité
        # Matrice P qui contient les proba de transition et qui est écrit en fonction de gamma
        P[0][1] = np.exp(gamma[0]) / (1 + np.exp(gamma[0]) + np.exp(gamma[1]))
        P[0][2] = np.exp(gamma[1]) / (1 + np.exp(gamma[0]) + np.exp(gamma[1]))
        P[1][0] = np.exp(gamma[2]) / (1 + np.exp(gamma[2]) + np.exp(gamma[3]))
        P[1][2] = np.exp(gamma[3]) / (1 + np.exp(gamma[2]) + np.exp(gamma[3]))
        P[2][0] = np.exp(gamma[4]) / (1 + np.exp(gamma[4]) + np.exp(gamma[5]))
        P[2][1] = np.exp(gamma[5]) / (1 + np.exp(gamma[4]) + np.exp(gamma[5]))
    ...
    def fit(self):
        ...
        # reinitialisation
        scaling_gamma = -2
        matrix_init = scaling_gamma * np.ones((self.k_regimes, self.k_regimes))

        matrix_prob = np.ones((self.k_regimes, self.k_regimes))
        for i in range(0, self.k_regimes):
            for j in range(0, self.k_regimes):
                matrix_prob[i][j] = np.exp(matrix_init[i][j])/(1+np.sum(np.exp(matrix_init[i]))-np.exp(matrix_init.diagonal()[i]))
                matrix_prob[np.diag_indices_from(matrix_prob)] = 1/(1+np.sum(np.exp(matrix_init[i]))-np.exp(matrix_init.diagonal()[i]))
        ...
        params_init = np.concatenate(
            (omega_init, alpha_init, beta_init, matrix_prob[~np.eye(matrix_prob.shape[0], dtype=bool)]
             .reshape(matrix_prob.shape[0], -1)
             .reshape((1 * (self.k_regimes * self.k_regimes-self.k_regimes))))
            )  # rajouter les mu

        res = minimize(self.log_likelihood, params_init, method='Nelder-Mead', options={"xtol": 1e-7, 'disp': True})  # maximise ?
